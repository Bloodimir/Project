...
namespace Bloodimir_Shen
{
    internal class Program
    {
        ...
        private static void Ee()
        {
            ...
            var predepos = E.GetPrediction(etarget);
            if (EMenu["e"].Cast<KeyBind>().CurrentValue)
            {
                if (E.IsReady())
            ...
            void Combo(bool useE, bool useW, bool useQ)
        {
            ...
            if (useE && E.IsReady())
            {
                var eTarget = TargetSelector.GetTarget(E.Range, DamageType.Magical, Player.Instance.ServerPosition);
                if (eTarget == null)
                {
                    return;
                }
                var predE = E.GetPrediction(eTarget);
                    if ((predE != null && eTarget.IsValidTarget() && EMenu["taunt" + eTarget.ChampionName].Cast<CheckBox>().CurrentValue && predE.GetCollisionObjects<AIHeroClient>().Count() >= EMenu["eslider"].Cast<Slider>().CurrentValue))
                            {
