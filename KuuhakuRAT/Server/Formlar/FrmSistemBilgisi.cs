...
{
    public partial class FrmSistemBilgisi : Form
    {
        private readonly Client _connectClient;

        public FrmSistemBilgisi(Client c)
        {
            _connectClient = c;
        ...
        private void FrmSystemInformation_Load(object sender, EventArgs e)
        {
            ...
            {
                this.Text = PencereYardımcısı.GetWindowTitle("Sistem Bilgisi", _connectClient);
                new KuuhakuCekirdek.Paketler.ServerPaketleri.GetSystemInfo().Execute(_connectClient);

                if (_connectClient.Value != null)
                {
                    ListViewItem lvi =
                        new ListViewItem(new string[] {"İşletim Sistemi", _connectClient.Value.IşletimSistemi});
                    lstSystem.Items.Add(lvi);
                    lvi =
                        new ListViewItem(new string[]
                        {
                            "Architecture",
                            (_connectClient.Value.IşletimSistemi.Contains("32 Bit")) ? "x86 (32 Bit)" : "x64 (64 Bit)"
                        });
                    lstSystem.Items.Add(lvi);
                    lvi = new ListViewItem(new string[] {"", "Daha çok bilgi toplanıyor..."});
                    lstSystem.Items.Add(lvi);
                }
            }
        }
        ...
        public void AddItems(ListViewItem[] lviCollection)
        {
            try
            {
                ...
                {
                    lstSystem.Items.RemoveAt(2); // Bilgi Yükleniyor
