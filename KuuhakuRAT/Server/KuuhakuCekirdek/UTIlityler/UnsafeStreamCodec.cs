    ...
    public class UnsafeStreamCodec : IDisposable
    {
            ...
            Stream outStream)
        {
            lock (_imageProcessLock)
            {
                ...
                {
                    throw new Exception("Akışa yazmak için erişimin olması gerekir");
                }
                ...
                {
                    throw new Exception("Piksel Formatı bir önceki Bitmapınkine eşit değildir.");
                }
                else if (this._encodedWidth != imageSize.Width || this._encodedHeight != imageSize.Height)
                {
                    throw new Exception("Bitmap yüksekliği/genişliği bir önceki Bitmaptakiyle eşit değildir.");
                }
